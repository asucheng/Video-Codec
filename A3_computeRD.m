function [J, encoded_block] = A3_computeRD(residual_block, mode, QP, Lambda)
    % Quantize and encode residuals
    [encoded_block, quantized_block] = A3_quantizeAndEncode(residual_block, QP);

    % Explicit rescaling to approximate original coefficients
    i = size(residual_block, 1); % Block size
    Q_Matrix = A3_generateQMatrix(i, QP); % Generate Q matrix
