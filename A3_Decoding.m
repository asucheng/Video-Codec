function psnrValues_verify = A3_Decoding(filename_prefix, nframes, blockSize, paddedWidth, paddedHeight, ...
    height, width, QP, nRefFrames, FMEEnable, FastME, RCflag)
    ...
    QPDiff_file = fopen(strcat(filename_prefix, "QPDiff.txt"), "r");
    ...
    if RCflag
        for frameIdx = 1:nframes
            ...
            for row = 1:blockSize:paddedHeight
                ...
                QPDiff_line = fgetl(QPDiff_file);
                if isempty(QPDiff_line)
                    error('Unexpected end of QPDiff file.');
                end
    
                QPDiff_line_array = A1_Q4_expGolombDecode(strtrim(QPDiff_line));
                QP = QPDiff_line_array(1) + previous_qp;
                previous_qp = QP;
