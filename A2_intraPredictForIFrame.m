function [predictedFrame, reconstructedFrame] = A2_intraPredictForIFrame(original_frame, block_size, QP, ...
    MDiff_stream, MVPDiff_stream, QTC_stream, ...
    VBSEnable, ~, FastME)

    Q_Matrix = A2_generateQMatrix(block_size, QP);
    ...
    for i = 1:block_size:size(original_frame, 1)
        for j = 1:block_size:size(original_frame, 2)
            ...
            [predicted_block, mode, split_flag] = A2_intraPredictForIBlock(original_frame, i, j, ...
            ...
            if VBSEnable && split_flag
                 ...
                 sub_Q_Matrix = A2_generateQMatrix(split_size, QP);
                ...
                for idx = 1:4
                    ...
                    [sub_predicted, sub_mode, ~] = A2_intraPredictForIBlock(original_frame, absSubRow, absSubCol, ...
                    ...
                    [encoded_value, ~] = A2_quantizeAndEncode(residual_block, QP, false);
                    ...
                    reconstructed_residual_block = A2_idctAfterDequantizeBlock(decoded_coeffs, sub_Q_Matrix);
                ...
                encoded_residual_block = A2_quantizeBlockAfterDCT(residual_block, Q_Matrix);
                scanned_coeffs = A1_Q4_sScan(encoded_residual_block);
                ...
                decoded_residual_block = A2_idctAfterDequantizeBlock(encoded_residual_block, Q_Matrix);
